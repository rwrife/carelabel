...
The fixture proves that `CareStoreSchema.migrator` can upgrade a real v1-era
...
Usage (repo root, after `swift build` of CareStore or via the Linux loop):
    Scripts/make_fixture_db.py <path-to-CareStore-clone-checkout-dir>

The helper locates the `fixture-seed` executable built from
Packages/CareStore (a tiny seed tool under Tests/CareStoreFixtureSeed).
"""
...
FIXTURE = REPO_ROOT / "Packages/CareStore/Tests/CareStoreTests/Fixtures/v1.sqlite"
